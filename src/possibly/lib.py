...
class Possibly:
    ...
    def configure_provider(self, provider, model=None, reasoning_effort=None, provider_config=None):
        """Apply process-only settings to a bound intelligence adapter for subsequent operations."""
        if self.intelligence is None:
            raise PossiblyError(
                "model_access_required", "Bind an intelligence adapter before configuring it."
            )
        return self.intelligence.configure_provider(provider, model, reasoning_effort, provider_config)
    ...
    def get_exploration(self, exploration_id):
        """Return an atomic snapshot and matching observation cursor; HTML is retrieved separately."""
    ...
    def save_review_state(
        ...
        *,
        ...
        if (
            ...
            raise PossiblyError("invalid_review", "Supply a reviewer ID and nonnegative sequence.")
        ...
        def accept(db):
            ...
            review = reviews.setdefault(reviewer_id, {"sequence": -1, "drafts": {}})
            if sequence <= review["sequence"]:
                return self._receipt(state, saved=False)
            review.update(sequence=sequence, view_id=view_id, revision_id=revision_id, saved_at=time.time())
    ...
    def read_artifact(self, exploration_id, revision_id):
        return self._revision(self.store.get(exploration_id), revision_id)["html"]

    def record_decision(
        self, exploration_id, revision_id, *, action, request_id, text="", expected_state_version=None
    ):
    ...
    def answer(self, exploration_id, operation_id, question_id, text, *, request_id):
        def accept(db):
            state = self.store.get(exploration_id, db)
            self._active(state)
    ...
    def export(self, exploration_id, revision_id, *, request_id):
        ...
        return self.store.mutate(
            self.caller, request_id, {"method": "export", "eid": exploration_id, "rid": revision_id}, accept
        )
